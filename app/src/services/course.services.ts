...
const updateCourseDetail = (data: any, id: any/*, token: string*/) => {
  console.log(`${backend_route}api/courses/update/${id}`)
  return axios.post(
    `${backend_route}/api/courses/update/${id}`, // TODO: change backend url to not include final /
    data/*,
    { headers: { Authorization: `Bearer ${token}` } }*/
  ).then(res => res.data);
}
...
const createSection = async (data: any, id: any, token: string) => {
  return await axios.post(
    `${backend_route}/api/sections/create/${id}`,
    data/*,
    { headers: { Authorization: `Bearer ${token}` }}*/
  );
}
