...
const UserDetailsModal: React.FC<UserDetailsModalProps> = ({
  ...
  const renderModal = () => {
    ...
    if (isRejectModalOpen || isApproveModalOpen) {
      return <GenericModalComponent {...modalProps} />;
    }
    return null;
  };

  const getCreatorStatusText = (creator: ContentCreator | null): string => {
    // If there's no creator data, the status is unknown or pending.
    if (creator === null) {
      return "Aguardando análise";
    }

    // Check statuses in a clear, prioritized order.
    if (creator.approved) {
      return "Aprovado";
    }
    ...
    // Default case if no other status matches.
    return "Aguardando análise";
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="flex flex-col items-center bg-white p-2 md:p-4 rounded-r-lg mt-4">
              ...
              <dd
                ...
                className="text-base font-['Montserrat'] text-gray-900 break-all"
              >
                {getCreatorStatusText(contentCreator)}
              </dd>
            </div>
          </div>
          <button
            ...
            className={`mt-4 py-4 px-2 rounded-lg text-base font-base font-['Lato'] w-full flex items-center transition-colors duration-300 ${
              isDropdownOpen[0]
                ...
                : "rounded-lg bg-white text-neutral-700"
            }`}
          >
            <svg
              ...
              userApplication.application.academicLevel.map((_, index) => (
                ...
                >
                  <p className="text-base font-['Montserrat'] text-gray-900">
                    ...
                    <br />
                    Major:{" "}
                    {Boolean(userApplication.application.major[index]) ||
                      "Not provided"}
                    <br />
                    Institution:{" "}
                    {Boolean(userApplication.application.institution[index]) ||
                      "Not provided"}
                    <br />
            ...
            {userApplication.application.company.length > 0 ? (
              userApplication.application.company.map((_, index) => (
                ...
                >
                  <p className="text-base font-['Montserrat'] text-gray-900">
                    Company:{" "}
                    {Boolean(userApplication.application.company[index]) ||
                      "Not provided"}
                    <br />
                    Position:{" "}
                    {Boolean(userApplication.application.position[index]) ||
                      "Not provided"}
                    <br />
                    Work activities:{" "}
                    {Boolean(
                    ...
                    <br />
                    Work end date:{" "}
                    {Boolean(userApplication.application.workEndDate[index]) ||
                      "Not provided"}
                  </p>
                </div>
              ))
