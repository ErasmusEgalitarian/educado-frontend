...
export const InstitutionsTableAdmin = () => {
  ...
  const { data: institutionsResponse, mutate } = useSWR(
    "api/institutions",
    () => getInstitutions(userToken)
  );

  if (!institutionsResponse) return <Loading />;

  const filteredData = institutionsResponse.filter((institution): boolean => {
      ...
      <table className="table w-full">
          ...
          <tr className="border-b-4 border-primary text-primary">
            {columnNames.map((columnName, key) => (
              <th
                ...
                key={`${columnName.name}-${String(key)}`}
              >
                {columnName.name}
              </th>
            ))}
        ...
        <tbody>
          {paginatedData.map((institution) => {
            return (
              <tr
                key={institution._id ?? institution.institutionName}
