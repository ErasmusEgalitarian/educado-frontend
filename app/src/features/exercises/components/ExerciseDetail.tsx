...
import { Exercise } from "../../../common/unknown/interfaces/Exercise";
...
export const ExerciseDetail = ({
  ...
  const onExerciseSave: SubmitHandler<any> = (data) => {
    updateExercise(data);
  };
  ...
  const deleteExercise = async () => {
    if (confirm("Você tem certeza?")) {
      ...
      if (status >= 200 && status <= 299) {
        ...
        toast.error(
          `(${status}, ${response.statusText}) while attempting to delete exercise`
        );
      }
    }
  };
  ...
  return (
    <form
      onSubmit={handleExerciseSave(onExerciseSave)}
